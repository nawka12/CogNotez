...
- Harden markdown rendering: add HTML sanitization before any `innerHTML` usage (note previews, AI chat bubbles) to close renderer XSS risk.
- Fix decryption passphrase modal template: remove embedded `const t = ...` inside HTML string and centralize modal templating to avoid inline JS leakage.
